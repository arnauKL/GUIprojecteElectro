...
# Classe que guarda la finestra
    ...
    def __init__(self):
        ...
        self._bar_refs = self.canvas_barplot.axes.bar(['SNS', 'PNS'], [0, 0], color=['tab:red', 'tab:blue'])

        self.dirty = False  # Per saber si tenim noves dades i s ha d actualitzar es gràfic
        ...
        self.timer.setInterval(DELAY_FRAMES)
    ...
    def update_plots(self):
        ...
        if not self.dirty:  # Abans d res comprovam q hi hagi noves dades a mostrar
            return
        
        # Primer mirem pel plot de l'ECG
        if self._plot_ecg_ref is None:  # Si no tenim referència al plot, el generem
        ...
        self.canvas_ecg.axes.autoscale(enable=True, axis='y')
        ...
        self.canvas_res.axes.autoscale(enable=True, axis='y')
        self.canvas_res.draw()


        # A update_plots:
        self._bar_refs[0].set_height(self.SNS_value)
